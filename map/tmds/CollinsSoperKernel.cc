...
double gK1(int const& nf, double const& lambda)
{
  ...
  return apfel::CF * apfel::KCS00() + b1 * apfel::CF * apfel::gammaK0() / bt0 * ( lambda + log( 1 - lambda ) ) / ( 1 - lambda )
...
double gK2(int const& nf, double const& lambda)
{
  ...
  return apfel::CF * apfel::KCS10(nf)
    ...
    + pow(b1, 2) * apfel::CF * apfel::gammaK0() / bt0 * ( pow(lambda, 2) - pow(log( 1 - lambda ), 2) ) / 2 / pow(1 - lambda, 2)
    + b1 * apfel::CF * apfel::gammaK1(nf) / bt0 * ( 2 * log( 1 - lambda ) - lambda * ( lambda - 2 ) ) / 2 / pow(1 - lambda, 2)
...
int main()
{
  ...
  // Get perturbative part of the Collin-Soper kernel
  const std::function<double(double const&, double const&)> CSKernel = apfel::CollinsSoperKernel(TmdObj, Alphas, PerturbativeOrder);

  // Get b* prescription
  //const std::function<double(double const&, double const&)> bs = NangaParbat::bstarMap.at("bstarmin");
  // const std::function<double(double const&, double const&)> bs = NangaParbat::bstarMap.at("bstarCSS");
  const std::function<double(double const&, double const&)> bs = [] (double const& bT, double const&) -> double { return bT; };

  // Non-perturbative part of the evolution (modify at will)
  const double g2 = 0; //0.2;
  const std::function<double(double const&)> NPEvol = [=] (double const& b) -> double{ return - g2 * pow(b, 2); };
