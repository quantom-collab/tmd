...
int main(int argc, char *argv[])
{
   ...
   if (argc < 4 || strcmp(argv[1], "--help") == 0) {
      std::cout << "\nUsage:" << std::endl;
      std::cout << "Syntax: ./SIDISMultiplicities <YAML configuration file [config.yaml]> <path to "
   ...
   for (auto const &exper : datasets)
      for (auto const &ds : exper.second) {
         std::cout << "\033[1;37m\nComputing SIDIS multiplicities for: "
         ...
         */
         ...
         const std::function<std::map<int, double>(double const &, double const &)> tPDFs =
             [&](double const &x, double const &Q) -> std::map<int, double> {
         ...
         const auto tRotPDFs = [=](double const &x, double const &mu) -> std::map<int, double> {
            return apfel::PhysToQCDEv(tPDFs(x, mu));
         };

         // EW charges (only photon contribution, i.e. only electric
         // charges squared).
         std::function<std::vector<double>(double const &)> fBq =
         ...
         if (pto > 1 || pto < 0) PerturbativeOrder++;
         if (pto > 2 || pto < -1) PerturbativeOrder++;

         // Initialize inclusive structure functions
         const auto IF2 = BuildStructureFunctions(
             InitializeF2NCObjectsZM(gpdf, Thresholds), tRotPDFs, PerturbativeOrder,
             [=](double const &Q) -> double {
                return TabAlphas.Evaluate(Q);
             },
             fBq);
         const auto IFL = BuildStructureFunctions(
             InitializeFLNCObjectsZM(gpdf, Thresholds), tRotPDFs, PerturbativeOrder,
             [=](double const &Q) -> double {
                return TabAlphas.Evaluate(Q);
             },
             fBq);
         ...
         const auto DiffInclusiveCS = [=](const double Q, const double x) -> double {
            ...
            std::cout << "xIntegrand: " << Yp * f2.Evaluate(x) / x - pow(Q / Vs, 4) * fl.Evaluate(x)
            / pow(x, 3) << std::endl;
            */

            return pow(TabAlphaem.Evaluate(Q), 2) / pow(Q, 3) *
                   (Yp * f2.Evaluate(x) / x - pow(Q / Vs, 4) * fl.Evaluate(x) / pow(x, 3));
         };
         ...
         for (int iqT = 0; iqT < (int)qTv.size(); iqT++) {
            ...
            const double differHERMES = apfel::ConvFact * apfel::FourPi * qTm *
                                        DEObj.transform(bIntProva, qTm) / (2 * Qm) / zm;
            const double differCOMPASS = apfel::ConvFact * apfel::FourPi * qTm *
                                         DEObj.transform(bIntProva, qTm) / (2 * Qm) /
                                         (2 * pow(zm, 2) * qTm);

            // Denominator
            const double denom =
                apfel::ConvFact * apfel::FourPi * DiffInclusiveCS(Qm, xm) / (2 * Qm);
   ...
   std::ofstream fout(OutputFolder + "/ReportMultiplicities" + suffix + ".yaml");
