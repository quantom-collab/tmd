...
int main(int argc, char *argv[])
{
   ...
   if (argc < 7 || strcmp(argv[1], "--help") == 0) {
      ...
      std::cout << "Syntax: ./PlotTMDs_bT <configuration file> <output file> <pdf/ff> <flavour ID> "
                   "<Scale in GeV> <parameters file>\n"
                << std::endl;
      exit(-10);
   }
   ...
   // Open the LHAPDF set
   LHAPDF::PDF *dist = LHAPDF::mkPDF(config[pf + "set"]["name"].as<std::string>(),
                                     config[pf + "set"]["member"].as<int>());
   ...
   for (int i = 0; i < nx; i++) {
      ...
      // Then, interpolate linearly in the logarithmic domain: log(x[i]) = log(x_min) + t *
      // (log(x_max) - log(x_min)) Finally, exponentiate to obtain the logarithmically spaced x
      // value.
   ...
   // Construct the x grid for APFEL++ from the configuration (this grid is used for the collinear
   // distributions)
   ...
   const auto Alphas = [&](double const &mu) -> double {
      return dist->alphasQ(mu);
   };
   const auto CollDists = [&](double const &mu) -> apfel::Set<apfel::Distribution> {
      return TabDists.Evaluate(mu);
   };

   std::function<apfel::Set<apfel::Distribution>(double const &, double const &, double const &)>
       EvTMDs;
   if (pf == "pdf")
   ...
   else throw std::runtime_error("[PlotTMDs]: Unknown distribution prefix");

   // b* prescription: lookup the b* function from NangaParbat
   const std::function<double(double const &, double const &)> bs =
       NangaParbat::bstarMap.at(config["bstar"].as<std::string>());
   ...
   NangaParbat::Parameterisation *NPFunc =
       NangaParbat::GetParametersation(parfile["Parameterisation"].as<std::string>());
   const std::vector<std::vector<double>> pars =
       parfile["Parameters"].as<std::vector<std::vector<double>>>();
