"""
Generate synthetic unpolarized SIDIS kinematic points. Synthetic refers to the fact that
the kinematics are not from real data, but are generated randomly within realistic kinematic ranges.

Creates mock kinematics with 4 columns [x, PhT, Q, z] in realistic kinematic ranges.
PhT respects the TMD boundary PhT/(z·Q) < 0.2 per event.

Saves both .dat and .pt files (same content, readable and PyTorch format).
Creates kin/ folder if it does not exist.
"""
...
def generate_mock_kinematics(n_events: int = 1000, seed: int = 42) -> torch.Tensor:
    """
    Generate synthetic SIDIS kinematic points

    Each event has 4 columns: [x, PhT, Q, z]. Values are drawn uniformly
    in realistic kinematic ranges (see below). PhT respects the TMD boundary
    PhT/(z*Q) < 0.2 for each event.

    Parameters
    ----------
    n_events : int
        Number of kinematic points to generate
    seed : int
    ...
    -------
    torch.Tensor
        Kinematic points tensor of shape (n_events, 4)
    """
...
EPILOG = """
Examples:
  # Default: 1000 events, seed 42, save to sidis/tests/kin/
  python sidis/tests/generate_mock_kinematics.py

  # 100 events with custom seed
  python sidis/tests/generate_mock_kinematics.py -n 100 -s 123

  # Custom output path (creates parent dirs; saves both .dat and .pt)
  python sidis/tests/generate_mock_kinematics.py -n 500 -o sidis/tests/kin/my_kinematics.dat

  # From sidis/tests/ directory
  python generate_mock_kinematics.py -n 10000 -o kin/mock_kinematics_10000
"""


def main():
    # Resolve the script directory and the kin/ directory
    script_dir = pathlib.Path(__file__).resolve().parent
    kin_dir = script_dir / "kin"

    parser = argparse.ArgumentParser(
        description=f"{tcolors.BOLDGREEN}Generates synthetic SIDIS kinematic points [x, PhT, Q, z]. {tcolors.ENDC}"
        f"{tcolors.BOLDWHITE}\nSaves both .dat and .pt (same content). \nCreates kin/ if missing.{tcolors.ENDC}",
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="Output file path without extension, or with .dat/.pt (default: kin/mock_kinematics_<n_points>)",
    )
    args = parser.parse_args()

    # Default output: kin/mock_kinematics_<n>
    if args.output is None:
        args.output = kin_dir / f"mock_kinematics_{args.n_events}"
    else:
    ...
    # Create kin/ (or output parent dir) if it does not exist
    args.output.parent.mkdir(parents=True, exist_ok=True)

    # Generate kinematics
    events = generate_mock_kinematics(n_events=args.n_events, seed=args.seed)
