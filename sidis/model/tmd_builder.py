...
class TMDBuilder(torch.nn.Module):
    ...
    def clear_cache(self):
        ...
        This method is called by TrainableModel when parameters change
        to ensure cached values are invalidated. Currently, TMDBuilder
        does not cache any computations (see class docstring), so this
        is a no-op. It exists to maintain the interface for potential
        future caching implementations.

        Note:
            This method is intentionally empty because:
            - Evolution factors are not cached (see class docstring)
            - Non-perturbative evolution factors are not cached
            - TMDs are not cached (depend on xi which varies per event)
        """
        pass
