...
def get_akima_derivatives_2d(
    x_grid: torch.Tensor, y_grid: torch.Tensor, z_grid: torch.Tensor
    ...
    w_x = (
        torch.abs(delta_x[1:] - delta_x[:-1])
        + torch.abs(delta_x[1:] + delta_x[:-1]) / 2
    )
    w1_x = w_x[:-2]
    w2_x = w_x[2:]
    ...
    w_y = (
        torch.abs(delta_y[:, 1:] - delta_y[:, :-1])
        + torch.abs(delta_y[:, 1:] + delta_y[:, :-1]) / 2
    )
    w1_y = w_y[:, :-2]
    w2_y = w_y[:, 2:]
    ...
    d_xy = (
        w2_x * (w2_y * delta_xy[1:-2, 1:-2] + w1_y * delta_xy[1:-2, 2:-1])
...
def interp_2d(
    ...
    y_grid: torch.Tensor,
    z_grid: torch.Tensor,
    dx_grid: torch.Tensor,
    dy_grid: torch.Tensor,
    dx_dy_grid: torch.Tensor,
    type: str = "cubic",
) -> torch.Tensor:
    """
    ...
    if type.lower() == "bilinear":
        ...
        coeff_mat = torch.stack(
            [
                torch.stack(
                    [
                        z_grid[idx_x1, idx_y1],
                        z_grid[idx_x1, idx_y2],
                        h_y * dy_grid[idx_x1, idx_y1],
                        h_y * dy_grid[idx_x1, idx_y2],
                    ],
                    axis=-1,
                ),
                torch.stack(
                    [
                        z_grid[idx_x2, idx_y1],
                        z_grid[idx_x2, idx_y2],
                        h_y * dy_grid[idx_x2, idx_y1],
                        h_y * dy_grid[idx_x2, idx_y2],
                    ],
                    axis=-1,
                ),
                torch.stack(
                    [
                        h_x * dx_grid[idx_x1, idx_y1],
                        h_x * dx_grid[idx_x1, idx_y2],
                        h_x_y * dx_dy_grid[idx_x1, idx_y1],
                        h_x_y * dx_dy_grid[idx_x1, idx_y2],
                    ],
                    axis=-1,
                ),
                torch.stack(
                    [
                        h_x * dx_grid[idx_x2, idx_y1],
                        h_x * dx_grid[idx_x2, idx_y2],
                        h_x_y * dx_dy_grid[idx_x2, idx_y1],
                        h_x_y * dx_dy_grid[idx_x2, idx_y2],
                    ],
                    axis=-1,
                ),
            ],
        ...
        theta_x_vec = torch.stack(
            [torch.ones(theta_x.shape), theta_x, theta_x**2, theta_x**3], axis=-1
        )
        theta_y_vec = torch.stack(
            [torch.ones(theta_y.shape), theta_y, theta_y**2, theta_y**3], axis=-1
        )
        ...
        return (
            theta_x_vec[:, None, None, :] @ M1 @ theta_y_vec[None, :, :, None]
        ).squeeze()
    else:
        raise ValueError(f"Invalid interpolation type: {type}")


def interp_2d_multiple_events(
    x: torch.Tensor,
    y: torch.Tensor,
    x_grid: torch.Tensor,
    y_grid: torch.Tensor,
